...
/*   Updated: 2016/04/07 16:25:03 by tgauvrit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	super_main( void ) {
	ClapTrap clap("BuddyBot");
	FragTrap frag("BoomBot");
	ScavTrap scav("Hinges");
	NinjaTrap ninja("Kazekage");
	fred_speaks("No wait, I have one more design left in me...");
	SuperTrap silver("Superman");
	SuperTrap shiny(silver);
	SuperTrap gold("Batman");
	shiny = gold;
	SuperTrap chromed(shiny);
	fred_speaks("Kill the remaining zombies on estate!");
	silver.meleeAttack("Zombie");
	chromed.takeDamage(28);
	shiny.takeDamage(1);
	...
	chromed.vaulthunter_dot_exe("Zombie");
	chromed.vaulthunter_dot_exe("Zombie");
	fred_speaks("Alright that's it, no more robots.");
}
